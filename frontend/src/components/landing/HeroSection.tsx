...
const HeroSection: React.FC = () => {
  ...
  return (
    <section className="relative isolate overflow-hidden bg-gradient-to-b from-bg via-surface to-bg">
      ...
      <div className="pointer-events-none absolute left-1/2 top-0 -z-10 h-[120%] w-[120%] -translate-x-1/2 rotate-45 transform bg-gradient-to-r from-primary/30 via-secondary/40 to-primary/30 opacity-30 blur-3xl" />
      ...
      <div className="mx-auto flex min-h-[80vh] max-w-screen-lg flex-col items-center justify-center px-6 pt-28 pb-20 text-center">
        ...
        >
          {t('hero.title_part1')}
          <br className="hidden sm:block" /> {t('hero.title_part2')}
        </h1>
        <p className="mt-6 max-w-2xl text-lg text-muted sm:text-xl">
          {t('hero.subtitle_before')} <span className="font-semibold text-text">{t('hero.subtitle_highlight')}</span> {t('hero.subtitle_after')}
        </p>
        <Link
          to="/auth"
        ...
        <div className="mt-20 flex motion-safe:animate-bounce flex-col items-center text-muted/60">
          ...
          <span className="mt-1 text-xs">{t('hero.more')}</span>
        </div>
      </div>
    </section>
  );
